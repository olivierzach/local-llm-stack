...
def main():
    ...
    result = subprocess.check_output(
        ["docker", "image", "inspect", pins["QWEN38_IMAGE"], "--format", "{{.Architecture}}"], text=True).strip()
    if result != "arm64":
        raise ValueError("pinned ARM64 image is not installed")
    ...
    print(json.dumps({"recipe": pins["QWEN38_RECIPE_REF"], "image": pins["QWEN38_IMAGE"],
