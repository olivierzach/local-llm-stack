...
def snapshot_check(snapshot):
    ...
    if 'Mistral3ForConditionalGeneration' in config.get('architectures', []):
        if not (snapshot / 'tekken.json').is_file():
            raise ValueError('missing Mistral tekken.json tokenizer')
    elif not (snapshot / 'tokenizer_config.json').is_file():
        raise ValueError('missing tokenizer_config.json')
...
def main():
    ...
    results = []
    auxiliary_errors = [name for name in catalog.get('auxiliary', []) if not (root / name).is_file()]
    ...
    report = {'models': results, 'missing_auxiliary': auxiliary_errors,
              'artifact_checks_passed': not auxiliary_errors and all(r['artifact_checks_passed'] for r in results),
