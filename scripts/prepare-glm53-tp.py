#!/usr/bin/env python3
"""Stage pinned GLM-5.3 TP artifacts on both Sparks; never start or stop a GPU workload.
...
def main():
    ...
    manifest_paths = [ROOT / 'cluster/model-downloads' / name for name in ('glm53-w4a16.json', 'glm53-dflash2.json')]
    manifests = [read(path) for path in manifest_paths]
    images_path = ROOT / 'cluster/glm53-images.lock.json'
    images = read(images_path)['images']
    total = sum(f['size'] for manifest in manifests for f in manifest['files'].values())
    print(json.dumps({'apply': args.apply, 'source': source['hostname'], 'peer': peer['hostname'],
        'models': [{'repo': m['repo'], 'revision': m['revision']} for m in manifests], 'download_bytes': total,
    ...
    with (output / 'prepare.lock').open('a') as lock:
        ...
        try:
            ...
            for manifest_path, manifest in zip(manifest_paths, manifests):
                model_output = output / manifest_path.stem
                ...
                run(['docker', 'run', '--rm', '--memory', '2g', '--cpus', '2',
                     ...
                     '-v', str(manifest_path) + ':/manifest.json:ro', '-v', str(model_output) + ':/receipts',
                ...
                with (model_output / 'model-copy.json').open('w') as receipt:
