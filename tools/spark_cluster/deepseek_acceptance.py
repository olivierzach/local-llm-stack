...
def verify(plan, directory):
    ...
    receipts = {}
    for name, relative in [('repeatability', 'repeatability.json'), ('tools', 'tools.json'),
    ...
    repeats = receipts['repeatability']
    require(repeats.get('passed') is True and len(repeats.get('records', [])) >= 100
            ...
            'publication requires 100 successful first-token regression trials')
